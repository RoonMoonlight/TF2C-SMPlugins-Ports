...
public void OnPluginStart()
{
	...
	g_precache = CreateConVar("spirt_dl_precache", "0", "(1 - enables | 0 - disabled) 1 - Enables file precaching. 0 - Disables file precaching (Disabled by default to avoid crashes)");
...
void PrepareFile(const char[] path)
{
	...
	else if (StrEqual(fileExtension, "mp3") || StrEqual(fileExtension, "wav") || StrEqual(fileExtension, "m4a") || StrEqual(fileExtension, "wmv") || StrEqual(fileExtension, "wma"))
	{
		CheckDownload(path);
	...
	else if (StrEqual(fileExtension, "txt"))
	{
		CheckDownload(path);
	}
