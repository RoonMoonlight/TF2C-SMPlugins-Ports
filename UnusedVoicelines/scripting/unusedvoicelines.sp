...
#define PLUGIN_VERSION "1.0_tf2c"

public Plugin myinfo =
{
	name = "[TF2C] Unused Voicelines",
	author = "Nanochip",
	description = "TF2C Port of Unused Voicelines by Nanochip. Plays unused announcer voicelines, class speeches, & sounds/music.",
...
char g_sClassNames[TFClassType][16] = { "Unknown", "Scout", "Sniper", "Soldier", "Demoman", "Medic", "Heavy", "Pyro", "Spy", "Engineer", "Civilian"};
...
void DontFailAgainLogic()
{
	...
	{
		if(GameRules_GetProp("m_bFourTeamMode")) // Check if current game mode is four team
		{
			// only pl and a/d swap teams on the next round.
			if (g_iLastTeamWon == 2)
			{
				PlaySound("dontFailAgain", 2);
			}
			if (g_iLastTeamWon == 3)
			{
				 PlaySound("dontFailAgain", 3);
			}
			if (g_iLastTeamWon == 4)
			{
				 PlaySound("dontFailAgain", 4);
			}
			if (g_iLastTeamWon == 5)
			{
				 PlaySound("dontFailAgain", 5);
			}
		}
		else
		{
...
public void OnSetupFinished(Event event, const char[] name, bool dontBroadcast)
{
	if (g_Gamemode == TF2_GameMode_PL && cvPayloadCart.BoolValue)
	{
		if(GameRules_GetProp("m_bFourTeamMode")) // Check if current game mode is four team
		{
			...
			PlaySound("payloadSetupFinishedGrn", 4);
			PlaySound("payloadSetupFinishedYlw", 5);
		}
		else
		{
			PlaySound("payloadSetupFinishedRed", 2);
	...
	if (GetClientCount() >= 10) // let's only play this sound if there's at least 10 people in the game
	{
		if(GameRules_GetProp("m_bFourTeamMode")) // Check if current game mode is four team
		{
			if (GetClientTeam(client) == 2 && GetPlayerCountTeam(2) <= 1)
			{
				...
				PlaySound("teamWipeOther", 4);
				PlaySound("teamWipeOther", 5);
			}
			...
			if (GetClientTeam(client) == 4 && GetPlayerCountTeam(4) <= 1)
			{
				g_bDoTeamWipe = false;
				PlaySound("teamWipe", 4);
				PlaySound("teamWipeOther", 2);
				PlaySound("teamWipeOther", 3);
				PlaySound("teamWipeOther", 5);
			}
			if (GetClientTeam(client) == 5 && GetPlayerCountTeam(5) <= 1)
			{
				g_bDoTeamWipe = false;
				PlaySound("teamWipe", 5);
				PlaySound("teamWipeOther", 2);
				PlaySound("teamWipeOther", 3);
				PlaySound("teamWipeOther", 4);
			}
			if (!g_bDoTeamWipe) CreateTimer(10.0, Timer_TeamWipeCooldown, _, TIMER_FLAG_NO_MAPCHANGE); // put a cooldown on this just in case it somehow gets spammed, since it is four team the cooldown is short
		}
		else
		{
			if (GetClientTeam(client) == 2 && GetPlayerCountTeam(2) <= 1)
...
void PlaySound(const char[] sound, int team = -1) //-1 = all, 2 = red, 3 = blu, 4 = grn, 5 = ylw
{
	char path[256];
	...
	if(GameRules_GetProp("m_bFourTeamMode")) // Check if current game mode is four team
	{
		...
		if (strcmp(sound, "payloadSetupFinishedGrn") == 0) Format(path, sizeof path, "vo/announcer_am_gamestarting03.mp3");
		if (strcmp(sound, "payloadSetupFinishedYlw") == 0) Format(path, sizeof path, "vo/announcer_am_gamestarting0%d.mp3", GetRandomInt(1, 2));
	}
	else
	{
		if (strcmp(sound, "payloadSetupFinishedRed") == 0) Format(path, sizeof path, "vo/announcer_am_gamestarting03.mp3");
