...
#define PLUGIN_VERSION			"1.3.0_tf2c"

public Plugin:myinfo = {
	name		= "[TF2C] Bot Manager",
	author		= "Dr. McKay, Roon",
	description	= "Port of Bot Manager to TF2C. Allows for customization of TFBots",
...
public APLRes:AskPluginLoad2(Handle:myself, bool:late, String:error[], err_max) {
	...
	if(!StrEqual(game, "tf2classic")) {
		strcopy(error, err_max, "Bot Manager: TF2C pOR only works on Team Fortress 2 Classic.");
...
AddBot() {
	...
	if(!GetConVarBool(cvarGameLogic)) {
		if(GameRules_GetProp("m_bFourTeamMode")) {
			if(GetTeamClientCount(2) < GetTeamClientCount(3) && GetTeamClientCount(2) < GetTeamClientCount(4) && GetTeamClientCount(2) < GetTeamClientCount(5)) {
				team = TFTeam_Red;
			} else if(GetTeamClientCount(3) < GetTeamClientCount(2) && GetTeamClientCount(3) < GetTeamClientCount(4) && GetTeamClientCount(3) < GetTeamClientCount(5)) {
				team = TFTeam_Blue;
			} else if(GetTeamClientCount(4) < GetTeamClientCount(2) && GetTeamClientCount(4) < GetTeamClientCount(3) && GetTeamClientCount(4) < GetTeamClientCount(5)) {
				team = TFTeam_Green;
			} else {
				team = TFTeam_Yellow;
			}
	...
	switch(team) {
		if(GameRules_GetProp("m_bFourTeamMode")) {
			...
			case TFTeam_Green: Format(strTeam, sizeof(strTeam), "green");
			case TFTeam_Yellow: Format(strTeam, sizeof(strTeam), "yellow");
...
RemoveBot() {
	...
	if(GameRules_GetProp("m_bFourTeamMode")) {
		if(GetTeamClientCount(2) > GetTeamClientCount(3) && GetTeamClientCount(2) > GetTeamClientCount(4) && GetTeamClientCount(2) > GetTeamClientCount(5)) {
			...
			teamToKick = 4;
		} else if(GetTeamClientCount(5) > GetTeamClientCount(2) && GetTeamClientCount(5) > GetTeamClientCount(3) && GetTeamClientCount(5) > GetTeamClientCount(4)) {
			teamToKick = 5;
		} else {
			teamToKick = GetRandomInt(2, 5);
		}
	} else {
		if(GetTeamClientCount(2) > GetTeamClientCount(3)) {
			teamToKick = 2;
