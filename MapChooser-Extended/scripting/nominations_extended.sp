...
public Action Command_NominateList(int client, int args)
{
	...
	ArrayList OwnerList = new ArrayList(arraySize);
	...
	if (client == 0)
	{
		...
		for(int i = 0; i < MapList.Length; i++)
		{
			...
			int index = OwnerList.Get(i);
			Format(owner, sizeof(owner), " %N", index);
			StrCat(aBuf, sizeof(aBuf), owner);
		}
	...
	for(int i = 0; i < MapList.Length; i++)
	{
		...
		Format(sBuffer, sizeof(sBuffer), "%s (%N)", map, index);

		AddMenuItem(NominateListMenu, map, sBuffer);
	}
