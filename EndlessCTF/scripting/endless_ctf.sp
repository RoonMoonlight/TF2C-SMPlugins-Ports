...
#define PLUGIN_VERSION   "1.2.0_tf2c"
...
public Plugin myinfo = {
	name = "[TF2C] Endless CTF",
	author = "Dr. McKay, Roon",
	description = "TF2C Port. Run Capture the Flag maps without actually ending the round",
...
int g_TeamResourceEntities[6] = {-1, ...};
...
public void OnMapStart() {
	...
	if (g_TeamResourceEntities[2] == -1 || g_TeamResourceEntities[3] == -1 || g_TeamResourceEntities[4] == -1 || g_TeamResourceEntities[5] == -1) {
		SetFailState("Could not find tf_team entities for teams 2/3/4/5");
	}
...
void CheckWinCondition() {
	...
	for (int team = 2; team <= 3; team++) {
		...
		if (captureCount >= capsRequired) {
			...
			SetEntProp(g_TeamResourceEntities[TFTeam_Green], Prop_Send, PROP_FLAG_CAPS, 0, 1);
			SetEntProp(g_TeamResourceEntities[TFTeam_Yellow], Prop_Send, PROP_FLAG_CAPS, 0, 1);
			int previousBlueScore = GetTeamScore(view_as<int>(TFTeam_Blue));
			int previousRedScore = GetTeamScore(view_as<int>(TFTeam_Red));
			int previousGreenScore = GetTeamScore(view_as<int>(TFTeam_Green));
			int previousYellowScore = GetTeamScore(view_as<int>(TFTeam_Yellow));
			...
			winEvent.SetInt("green_score", GetTeamScore(view_as<int>(TFTeam_Green)));
			winEvent.SetInt("yellow_score", GetTeamScore(view_as<int>(TFTeam_Yellow)));
			winEvent.SetInt("blue_score_prev", previousBlueScore);
			winEvent.SetInt("red_score_prev", previousRedScore);
			winEvent.SetInt("green_score_prev", previousGreenScore);
			winEvent.SetInt("yellow_score_prev", previousYellowScore);
